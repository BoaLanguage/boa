...
let int_of_value (v : value) : int =
  match v with
  | VInt i -> i
  | _ ->  raise @@ TypecheckerFail "Expected integer" 

let bool_of_value (v : value) : bool = 
  match v with
  | VBool b -> b
  | _ -> raise @@ TypecheckerFail "Expected boolean"
...
and evals (conf:configuration) : env =  
  ...
  let evaluate_assignment identifier expression =  
    ...
    | Some (VRef r) -> r := Some new_value; sigma 
    | _ -> (identifier, new_value)::sigma in 
    evals (new_sigma, next_statement, Pass, kappa) in
  let evaluate_attribute_assignment object_expression attribute_identifier new_value_expression =
    ...
    | _ -> raise @@ TypecheckerFail "Not an object" in
  let evaluate_member_declaration identifier = 
    let attrs = lookup sigma "__attrs__" in 
    match attrs with 
    | VList lst -> 
      let new_sigma = update sigma "__attrs__" (VList((VString(identifier))::lst)) in 
       evals (new_sigma, Pass, next_statement, kappa) 
    | _ -> raise @@ TypecheckerFail "__attrs__ is not a list" in
  let evaluate_mutable_member_declaration identifier = 
    let mattrs = lookup sigma "__mattrs__" in  
    match mattrs with 
    | VList lst -> 
      let new_sigma = update sigma "__mattrs__" (VList(((VString(identifier))::lst))) in 
        evals (new_sigma, Pass, next_statement, kappa)  
    | _ -> raise @@ TypecheckerFail "__mattrs__ is not a list" in
  let evaluate_if guard statement_true statement_false = 
    let b_result = evale guard sigma in
    match b_result with
    | VBool b' -> if b' 
      then evals (sigma, statement_true, next_statement, kappa) 
      else evals (sigma, statement_false, next_statement, kappa)
    | _-> raise @@ TypecheckerFail "If guard must be boolean" in
  ...
  let evaluate_class class_identifier superclass statement = 
    let new_sigma = ("__mattrs__", VList([]))::("__attrs__", VList([]))::sigma in
      ...
      evals ((class_identifier, VRef (ref (Some(clobj))))::sigma, next_statement, Pass, kappa) in
  match current_statement, next_statement with
  | Pass, Pass -> sigma
  | Pass, c -> evals (sigma, c, Pass, kappa)
  | Assign(v, a), _ -> evaluate_assignment v a
  | AttrAssgn(objexp, attr, e), c -> evaluate_attribute_assignment objexp attr e
  | Decl (_, _), c -> evals (sigma, Pass, c, kappa)
  | MutableDecl (_, v), c -> evals ((v, VRef (ref None))::sigma, c, Pass, kappa)
  ...
  | Print(a), c -> 
    Pprint.print_value @@ evale a sigma; Format.printf "%s" "\n"; 
...
and call (callable : value) (args : value list) : value = 
  ...
  let call_method obj closure = 
    ...
    begin
      match !r with
      | Some v -> v
      | _ -> raise @@ TypecheckerFail "Method returned uninitialized reference"
    end
    | v -> v in
  let call_closure params body env_ref = 
    let callenv = 
      match args with 
      | [] -> !env_ref
      | _ -> (zip params args)@(!env_ref) in 
      lookup (evals (callenv, body, Pass, [])) "return" in
  let call_reference r = 
    match !r with 
    | Some (VObj(cls_obj) as v) -> call v args
    | _ -> raise @@ TypecheckerFail "Callable is uninitialized" in
  let call_init class_object = 
    ...
    | VClosure (params, body, env) as init -> 
      ...
      let extract_string_list vlist = 
        ...
        | VList (val_list) -> 
          let extract_string vstring = 
            match vstring  with
            | VString s -> s
            | _ -> raise @@ TypecheckerFail "Atrribute names must be strings" in
          List.map extract_string val_list
        | _ -> raise @@ TypecheckerFail "Attributes must be a list" in 
      ...
      let cls_ref = [("__class__", ref (VRef (ref (Some (VObj class_object)))))] in
      ...
      begin
        ...
        | VPreObj(attrsmattrs) -> 
          ...
          let mattrs =  List.map (fun (name, vref) -> (name, VRef (ref (Some !vref)))) mattrs in
            VObj([("__class__", VRef(ref (Some (VObj(class_object)))))]@attrs@mattrs)
